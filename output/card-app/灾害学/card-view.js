...
(function() {
  ...
  function formatRelations(card) {
    if (!card.relations || card.relations.length === 0) {
      // 后向兼容：使用 knowledge_interfaces
      return formatInterfacesLegacy(card);
    }

    var APP = window.__APP__;
    var nodes = card.relations;
    var total = nodes.length;
    var centerX = 120, centerY = 120;
    var radius = Math.min(90, 40 + total * 22);

    var svgLines = '';
    var nodeItems = '';

    // 当前卡片（居中）
    nodeItems += '<div class="relation-node current" style="left:' + (centerX - 60) + 'px;top:' + (centerY - 18) + 'px">' +
                 AppState.escapeHtml(card.title) + '</div>';
    ...
    for (var i = 0; i < total; i++) {
      ...
      // 找匹配索引
      var targetIndex = -1;
      APP.cardData.forEach(function(c, idx) {
        if (c.title === rel.targetId) targetIndex = idx;
      });

      var relType = rel.type || 'expand';
      // SVG 连线
      svgLines += '<line x1="' + centerX + '" y1="' + centerY + '" x2="' + nx + '" y2="' + ny +
                  '" class="relation-line rel-line rel-type-' + relType + '" data-from="0" data-to="' + (i + 1) + '"/>';

      // 关系标签（连线中点偏移 8px 上）
      var lx = (centerX + nx) / 2;
      var ly = (centerY + ny) / 2 - 8;
      var labelMap = { expand: '展开', contrast: '对比', generalize: '概括', theorize: '理论', apply: '实践', precede: '前置', succeed: '后继' };
      svgLines += '<text x="' + lx + '" y="' + ly + '" class="relation-label" text-anchor="middle">' +
                  (labelMap[relType] || relType) + '</text>';

      // 节点
      nodeItems += '<div class="relation-node" data-index="' + targetIndex +
                   '" style="left:' + (nx - 60) + 'px;top:' + (ny - 14) + 'px">' +
                   AppState.escapeHtml(rel.targetId) + '</div>';
    }

    var html = '<div class="relation-graph" style="position:relative;min-height:' + (centerY * 2 + 20) + 'px">';
    html += '<svg class="relation-svg" width="' + (centerX * 2) + '" height="' + (centerY * 2) + '">' + svgLines + '</svg>';
    html += nodeItems;
    html += '</div>';
  ...
  // 后向兼容：旧版知识接口
  function formatInterfacesLegacy(card) {
  ...
  document.getElementById('cardSections').addEventListener('click', function(e) {
    // 关联图节点点击（径向图）
    var relNode = e.target.closest('.relation-node:not(.current)');
    if (relNode) {
      var index = parseInt(relNode.dataset.index);
